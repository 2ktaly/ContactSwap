    ...
    var body: some View {
        NavigationStack {
            ...
            .toolbar {
                ...
                ToolbarItem(placement: .topBarTrailing) {
                    Button("Leeren") {
                        // Bewusst nicht ausgegraut: Ein toter Knopf ohne
                        // Erklärung lässt den Nutzer im Dunkeln, seit der
                        // Hinweis nicht mehr in der Liste steht.
                        if isBlocked {
                            store.alert = .swapUsedUp
                        } else {
